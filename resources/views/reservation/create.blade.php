        ...
        <div class="flex justify-between md:flex-row flex-col ">
            ...
            <div class="md:w-2/3  md:border-r border-gray-800 p-2">
                ...
                <div class=" flex items-end mt-8 ms-4">
                    ...
                    <p>
                        <span class="text-3xl font-bold text-blue-500 ms-3 me-1 border border-blue-500 p-2 rounded-md">
                            {{ 'Rp ' . number_format($car->price_per_day, 0, ',', '.') }}
                        </span>
                        <span class="text-lg font-medium text-red-500 line-through">
                            {{ 'Rp ' . number_format(intval(($car->price_per_day * 100) / (100 - $car->reduce)), 0, ',', '.') }}
                        </span>                        
                    </p>
                </div>
                ...
                <div class="px-6 md:me-8">
                    <form id="reservation_form" action="{{ route('car.reservationStore', ['car' => $car->id]) }}"
                        ...
                        <div class="mt-12 md:block hidden  ">
                            <button type="submit"
                                class="text-white bg-blue-500 p-3 w-full rounded-lg font-bold hover:bg-black shadow-xl hover:shadow-none ">Order
                                Now</button>
                        </div>
                    </form>
                </div>
            ...
            <div class="md:w-1/3 flex flex-col justify-start items-center">
                <div class="relative mx-3 mt-3 flex h-[200px] w-3/4   overflow-hidden rounded-xl shadow-lg">
                    ...
                    <span
                        class="absolute w-24 h-8 py-1 top-0 left-0 m-2 rounded-full bg-blue-500 px-2 text-center text-sm font-medium text-white">{{ $car->reduce }}
                        %
                        OFF</span>
                </div>
                ...
                <div class="mt-3 ms-4 md:block hidden">
                    <div class="flex items-center">
                        @for ($i = 0; $i < $car->stars; $i++)
                            <svg aria-hidden="true" class="h-4 w-4 text-blue-500" fill="currentColor" viewBox="0 0 20 20"
                        ...
                        @endfor
                        <span
                            class="mr-2 ml-3 rounded bg-blue-500 px-2.5 py-0.5 text-sm font-semibold">{{ $car->stars }}.0</span>
                    </div>
                </div>
                ...
                <div class=" w-full   mt-8 ms-8">
                    <p id="duration" class="font-car text-gray-600 text-lg ms-2">Estimated Duration: <span
                            class="mx-2 font-car text-md font-medium text-gray-700 border border-blue-500 p-2 rounded-md "> --
                            days</span>
                    </p>
                </div>
                ...
                flatpickrElement._flatpickr.config.onChange.push(function(selectedDates, dateStr, instance) {
                    if (selectedDates.length === 2) {
                        ...
                        if (startDate && endDate && startDate <= endDate) {
                            ...
                            $('#total-price span').text(new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(totalPrice));
                        } else {
